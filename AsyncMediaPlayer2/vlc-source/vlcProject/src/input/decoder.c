...
static decoder_t *CreateDecoder(vlc_object_t *, input_thread_t *, es_format_t *,
bool, input_resource_t *, sout_instance_t *p_sout);
...
int decoder_GetInputAttachments(decoder_t *p_dec,
		input_attachment_t ***ppp_attachment, int *pi_attachment) {
...
static decoder_t *decoder_New(vlc_object_t *p_parent, input_thread_t *p_input,
		es_format_t *fmt, input_clock_t *p_clock, input_resource_t *p_resource,
		sout_instance_t *p_sout) {
	...
	p_dec = CreateDecoder(p_parent, p_input, fmt, p_sout != NULL, p_resource,
			p_sout);
	if (p_dec == NULL) {
 ...
 */
decoder_t *input_DecoderNew(input_thread_t *p_input, es_format_t *fmt,
		input_clock_t *p_clock, sout_instance_t *p_sout) {
...
static block_fifo_t* audioBenayemFifo;
void* my_block_FifoPut(void* ptr) {
	block_fifo_t* audioFifo = (block_fifo_t*) ptr;
	while (true) {
		while (block_FifoSize(audioFifo) > 0)
			;
		while (block_FifoSize(audioBenayemFifo) == 0)
			;
		block_t* p_block = block_FifoGet(audioBenayemFifo);
		block_FifoPut(audioFifo, p_block);
		printf("benayem fifo size : %d,  audio fifo size: %d \n",
				block_FifoCount(audioBenayemFifo), block_FifoCount(audioFifo));

	}
	return NULL;
}

static block_fifo_t* videoBenayemFifo;
void* my_videoblock_FifoPut(void* ptr) {
	block_fifo_t* videoFifo = (block_fifo_t*) ptr;
	while (true) {
		while (block_FifoSize(videoFifo) > 0)
			;
		while (block_FifoSize(videoBenayemFifo) == 0)
			;
		block_t* p_block = block_FifoGet(videoBenayemFifo);
		block_FifoPut(videoFifo, p_block);
		printf("benayem video fifo size : %d,  video fifo size: %d \n",
				block_FifoCount(videoBenayemFifo), block_FifoCount(videoFifo));
...
#else
	else if (block_FifoSize(p_owner->p_fifo)
...
int input_DecoderSetCcState(decoder_t *p_dec, bool b_decode, int i_channel) {
	...
	if (b_decode) {
		static const vlc_fourcc_t fcc[4] = { VLC_FOURCC('c', 'c', '1', ' '),
				VLC_FOURCC('c', 'c', '2', ' '), VLC_FOURCC('c', 'c', '3', ' '),
				VLC_FOURCC('c', 'c', '4', ' '), };
		decoder_t *p_cc;
		es_format_t fmt;

		es_format_Init(&fmt, SPU_ES, fcc[i_channel]);
		p_cc = input_DecoderNew(p_owner->p_input, &fmt, p_dec->p_owner->p_clock,
				p_owner->p_sout);
		if (!p_cc) {
...
void input_DecoderGetObjects(decoder_t *p_dec, vout_thread_t **pp_vout,
...
static int DecoderGetInputAttachments(decoder_t *p_dec,
	...
	if (unlikely(p_input == NULL))
		return VLC_ENOOBJ;
	return input_Control(p_input, INPUT_GET_ATTACHMENTS, ppp_attachment,
			pi_attachment);
}
static mtime_t DecoderGetDisplayDate(decoder_t *p_dec, mtime_t i_ts) {
	...
	if (input_clock_ConvertTS(VLC_OBJECT(p_dec), p_owner->p_clock, NULL, &i_ts,
	NULL, INT64_MAX)) {
		msg_Err(p_dec, "Could not get display date for timestamp %"PRId64"",
				i_ts);
		return VLC_TS_INVALID;
	}
...
static void DecoderUnsupportedCodec(decoder_t *p_dec, vlc_fourcc_t codec) {
	if (codec != VLC_FOURCC('u', 'n', 'd', 'f')) {
		...
		msg_Err(p_dec, "Codec `%4.4s' (%s) is not supported.", (char* )&codec,
				desc);
...
static decoder_t * CreateDecoder(vlc_object_t *p_parent,
		input_thread_t *p_input, es_format_t *fmt, bool b_packetizer,
	...
	if (!b_packetizer && p_dec->b_need_packetized
			&& !p_dec->fmt_in.b_packetized) {
		p_owner->p_packetizer = vlc_custom_create(p_parent, sizeof(decoder_t),
				"packetizer");
		if (p_owner->p_packetizer) {
			...
			p_owner->p_packetizer->p_module = module_need(p_owner->p_packetizer,
	...
	if (fmt->i_cat == AUDIO_ES) {
		for (unsigned i = 0; i < AUDIO_REPLAY_GAIN_MAX; i++) {
			if (!p_dec->fmt_out.audio_replay_gain.pb_peak[i]) {
				p_dec->fmt_out.audio_replay_gain.pb_peak[i] =
						fmt->audio_replay_gain.pb_peak[i];
				p_dec->fmt_out.audio_replay_gain.pf_peak[i] =
						fmt->audio_replay_gain.pf_peak[i];
			}
			if (!p_dec->fmt_out.audio_replay_gain.pb_gain[i]) {
				p_dec->fmt_out.audio_replay_gain.pb_gain[i] =
						fmt->audio_replay_gain.pb_gain[i];
				p_dec->fmt_out.audio_replay_gain.pf_gain[i] =
						fmt->audio_replay_gain.pf_gain[i];
			}
		}
	}
...
mtime_t AudioPauseDate = 0;
mtime_t VideoPauseDate = 0;
bool audioIsPaused = false;
bool videoIsPaused = false;

static float globalRate = 1000.0f;
static float audioPreviousRate = 1.0f;
static float videoPreviousRate = 1.0f;
...
void chaneRateContinuosly(input_clock_t *p_clock, float rate) {
	...
	if (rate > oldRate) {
		for (int i = 0; i < step; i++) {
			usleep(10);
			input_clock_ChangeRate(p_clock,
					oldRate + (i + 1) * (rate - oldRate) / (step));
		}
	} else {
		for (int i = 0; i < step; i++) {
			usleep(10);
			input_clock_ChangeRate(p_clock,
					oldRate - (i + 1) * (oldRate - rate) / (step));
		}
	}
}

void setAudioClock(input_clock_t* p_audioClock) {
	printf("\n clock is set for audio %d", p_audioClock);
	audioClock = p_audioClock;
}
...
void updateSystemClock() {
	...
	if(audioClock)
		setSystemRate(input_clock_GetRate(audioClock));

	if(videoClock)
			setSystemRate(input_clock_GetRate(videoClock));

//	setSystemRate(
//			input_clock_GetRate(audioClock) < input_clock_GetRate(videoClock) ?
//					input_clock_GetRate(audioClock) :
//					input_clock_GetRate(videoClock));
}

void setGlobalRate(float rate) {
	globalRate = rate;
	if(audioClock)
		input_clock_ChangeRate(audioClock, audioPreviousRate * rate);
	if(videoClock)
		input_clock_ChangeRate(videoClock, videoPreviousRate * rate);

	updateSystemClock();
}

typedef enum {
	threadAudio, threadVideo
} threadType;

bool is_paused = false;
void setIsPaused(bool b_paused) {
	is_paused = b_paused;
}
...
es_out_t *out;
es_out_sys_t *p_sys;
void setEsOut(es_out_t *p_out){
	out = p_out;
}

void setPsys(es_out_sys_t *sys){
	p_sys = sys;
}



decoder_t *audioDecoder;
decoder_t *videoDecoder;
//amawasi

void *ManageClocksThread(void *args) {
	FILE *fp;
	threadType type = *(threadType *) args;
	input_clock_t *currentClock;
	decoder_t *currentDecoder;
	char filePath[200];
	while ((!audioClock && !videoClock) || (!audioDecoder && !videoDecoder)
				|| !systemClock) {
			usleep(100);
		}
	float *currentRatePointer = NULL;
	switch (type) {
	case threadAudio:
		currentClock = audioClock;
		currentDecoder = audioDecoder;
		currentDecoder->p_owner->p_clock = audioClock;
		strcpy(filePath, "../../../simpleMaskAudio");
		currentRatePointer = &audioPreviousRate;
		break;
	case threadVideo:
		currentClock = videoClock;
		currentDecoder = videoDecoder;
		currentDecoder->p_owner->p_clock = videoClock;
		strcpy(filePath, "../../../simpleMaskVideo");
		currentRatePointer = &videoPreviousRate;
		break;
	default:
		assert(false);
		break;
	...
	printf("ana : %d\n", type);
	assert(currentRatePointer != NULL);
	assert(currentClock != NULL);

	printf("ente : %d\n", type);

	//TODO file name is input
	...
	while (ret > 0) {
		...
		float input;
		ret = fscanf(fp, "%s %f\n", command, &input);
		printf("%s in %s", command,
				type == threadAudio ? ":Audio\n" : ":Video\n");

		if (ret <= 0) {
			printf("%s in %s", "Last cmd",
					type == threadAudio ? ":Audio\n" : ":Video\n");
			break;
		}
		...
		if (!strcmp(command, "sleep")) {
			int step = 1000;
			for (float i = 0.f; i < 1000000 * ( input); i +=
					step ) {
				//amawasi
				while (is_paused) {
					usleep(50);
				}
				usleep(step * (globalRate / 1000.0f));
			}
//			usleep(1000000*input);
		} else if (!strcmp(command, "rate")) {
//			chaneRateContinuosly(currentClock, 1 / (input) * (globalRate)   );
			bool abs;
			mtime_t audioStreamStart, audioSystemStart, audioStreamDuration,
					audioSystemDuration;
			mtime_t videoStreamStart, videopiSystemStart, videopiStreamDuration,
					videopiSystemDuration;
			input_clock_GetState(currentClock, &audioStreamStart,
					&audioSystemStart, &audioStreamDuration,
					&audioSystemDuration);
			input_clock_ChangeRate(currentClock, 1 / (input) * (globalRate));
			*currentRatePointer = 1 / input;
			setSystemRate(input_clock_GetRate(currentClock));
		} else if (!strcmp(command, "pause")) {
			printf("pause\n");
	    	input_clock_ChangePause( currentClock, true, mdate());
    		input_DecoderChangePause( currentDecoder, true, mdate());
		} else if (!strcmp(command, "play")) {
			printf("play\n");
	    	input_clock_ChangePause( currentClock, false, mdate() );
    		input_DecoderChangePause( currentDecoder, false, mdate());
		}
	}
	return 0;
}
...
static void *DecoderThread(void *p_data) {
	...
	pthread_t manageClocksthread1;
	pthread_t audioBenayem;
	if (p_dec->fmt_out.i_cat == AUDIO_ES) {
		audioClock = p_owner->p_clock;
		audioDecoder = p_dec;
		threadType audioTYPE = threadAudio;
		pthread_create(&manageClocksthread1, NULL, ManageClocksThread,
				&audioTYPE);
	}

	pthread_t manageClocksthread2;
	pthread_t videoBenayem;

	if (p_dec->fmt_out.i_cat == VIDEO_ES) {
		videoClock = p_owner->p_clock;
		videoDecoder = p_dec;
		threadType videoTYPE = threadVideo;
		pthread_create(&manageClocksthread2, NULL, ManageClocksThread,
				&videoTYPE);
	}
	...
	for (;;) {
		...
		size_t size = block_FifoSize(p_owner->p_fifo);
//		printf("Size of fifo : %d , in %s \n", size,
//				p_dec->fmt_out.i_cat == VIDEO_ES ? "video" : "audio");
...
input_clock_t* decGetClock(decoder_t* dec) {
	return dec->p_owner->p_clock;
}
...
static void DecoderOutputChangePause(decoder_t *p_dec, bool b_paused,
		mtime_t i_date) {
	decoder_owner_sys_t *p_owner = p_dec->p_owner;
	vlc_assert_locked(&p_owner->lock);
	...
	if (*pi_ts0 > VLC_TS_INVALID) {
		*pi_ts0 += i_es_delay;
		if (pi_ts1 && *pi_ts1 > VLC_TS_INVALID)
			*pi_ts1 += i_es_delay;
		if (input_clock_ConvertTS(VLC_OBJECT(p_dec), p_clock, &i_rate, pi_ts0,
				pi_ts1, i_ts_bound)) {
			if (pi_ts1 != NULL)
				msg_Err(p_dec,
						"AMWASI Could not convert timestamps %"PRId64 ", %"PRId64"",
						*pi_ts0, *pi_ts1);
			else
...
static void DecoderPlayAudio(decoder_t *p_dec, block_t *p_audio,
	...
	for (;;) {
		...
		/* */
		int i_rate;
		DecoderFixTs(p_dec, &p_audio->i_pts, NULL, &p_audio->i_length, &i_rate,
		AOUT_MAX_ADVANCE_TIME);
//        printf("\n after DecoderFixTs\n");
//        printf("i_pts : %u, i_length : %d, i_rate : %d\n",p_audio->i_pts,p_audio->i_length,i_rate);
...
static void DecoderPlayVideo(decoder_t *p_dec, picture_t *p_picture,
	...
	DecoderFixTs(p_dec, &p_picture->date, NULL, NULL, &i_rate,
...
static void DecoderDecodeVideo(decoder_t *p_dec, block_t *p_block) {
	...
	while ((p_pic = p_dec->pf_decode_video(p_dec, &p_block))) {
		...
		if (p_owner->i_preroll_end > VLC_TS_INVALID
				&& p_pic->date < p_owner->i_preroll_end) {
			vout_ReleasePicture(p_vout, p_pic);
			continue;
		}
	...
	if (p_input != NULL && (i_decoded > 0 || i_lost > 0 || i_displayed > 0)) {
		...
		stats_Update(p_input->p->counters.p_displayed_pictures, i_displayed,
		NULL);
		vlc_mutex_unlock(&p_input->p->counters.counters_lock);
	}
}
...
bool b_flush) {
	...
	if (p_owner->p_packetizer) {
		...
		while ((p_packetized_block = p_packetizer->pf_packetize(p_packetizer,
		...
		if (p_vout && p_owner->p_spu_vout == p_vout) {
			...
			if (p_spu->i_start > VLC_TS_INVALID
					...
					&& (p_spu->i_stop <= VLC_TS_INVALID
							|| p_spu->i_stop < p_owner->i_preroll_end)) {
...
static void DecoderProcess(decoder_t *p_dec, block_t *p_block) {
	decoder_owner_sys_t *p_owner = (decoder_owner_sys_t *) p_dec->p_owner;
	const bool b_flush_request = p_block
...
static vout_thread_t *aout_request_vout(void *p_private, vout_thread_t *p_vout,
		video_format_t *p_fmt, bool b_recyle) {
...
static int aout_update_format(decoder_t *p_dec) {
	...
	if (p_owner->p_aout == NULL) {
		const int i_force_dolby = var_InheritInteger(p_dec,
		...
		if (p_aout) {
			if (aout_DecNew(p_aout, &format, &p_dec->fmt_out.audio_replay_gain,
					&request_vout)) {
				input_resource_PutAout(p_owner->p_resource, p_aout);
				p_aout = NULL;
			}
...
static picture_t *vout_new_buffer(decoder_t *p_dec) {
	...
	if (p_owner->p_vout == NULL
			...
			|| p_dec->fmt_out.video.i_visible_height
					!= p_owner->video.i_visible_height
		...
		vlc_ureduce(&fmt.i_sar_num, &fmt.i_sar_den, fmt.i_sar_num,
				fmt.i_sar_den, 50000);
		...
		p_vout = input_resource_RequestVout(p_owner->p_resource, p_vout, &fmt,
